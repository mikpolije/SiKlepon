...
class RecipeController extends Controller
{
    ...
    public function print(Checkup $checkup){
        try {
            ...
            $this->param['getMedicineRule'] = MedicineRule::all();
            $this->param['getDetailRecipe'] = \DB::table('recipe_details')
                                                ->select('recipes.code_rp', 'recipes.date_recipe', 'medicines.price', 'users.name as doctor_name', 'medicines.name as medicine_name', 'recipe_details.qty', 'recipe_details.medication_rules', 'recipe_details.total')
                                                ->join('recipes', 'recipe_details.recipe_id', 'recipes.id')
                                                ->join('medicines', 'recipe_details.medicine_id', 'medicines.id')
                                                ->join('users', 'recipes.doctor_id', 'users.id')
                                                ->where('recipes.checkup_id', $checkup->id)
            ...
            $pdf = PDF::loadview('doctor.pages.recipe.print', $this->param);
